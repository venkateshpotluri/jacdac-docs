(self["webpackChunkjacdac_docs"] = self["webpackChunkjacdac_docs"] || []).push([[4087],{
...
/* harmony export */ });
/* unused harmony exports unitDescription, secondaryUnitConverters, capitalize, snakify, dashify, humanify, TYPESCRIPT_STATIC_NAMESPACE, packFormat, packInfo, converters, isNumericType */
...
function toPython(info) {
  var r = ["# Autogenerated file for " + info.name, "# Add missing from ... import const"];
  var pref = "JD_" + toUpper(info.shortName) + "_";
  if (info.shortId[0] == "_") pref = "JD_";
  if (info.shortId[0] != "_") r.push("_JD_SERVICE_CLASS_" + toUpper(info.shortName) + " = const(" + toHex(info.classIdentifier) + ")");

  for (var cst in info.constants) {
    var _info$constants$cst = info.constants[cst],
        value = _info$constants$cst.value,
        hex = _info$constants$cst.hex;
    r.push("_JD_" + toUpper(cst) + " = const(" + (hex ? value.toString() : toHex(value)) + ")\n");
  }
...
function toH(info) {
  ...
  for (var cst in info.constants) {
    var _info$constants$cst2 = info.constants[cst],
        value = _info$constants$cst2.value,
        hex = _info$constants$cst2.hex;
    r += "#define " + pref + toUpper(cst) + " " + (hex ? toHex(value) : value.toString()) + "\n";
  }

  for (var _iterator9 = _createForOfIteratorHelperLoose(values(info.enums).filter(function (en) {
    return !en.derived;
    ...
    for (var _i4 = 0, _Object$keys4 = Object.keys(en.members); _i4 < _Object$keys4.length; _i4++) {
      var k = _Object$keys4[_i4];
      r += "#define " + enPref + "_" + toUpper(k) + " " + toHex(en.members[k]) + "\n";
    }
  }

  for (var _iterator10 = _createForOfIteratorHelperLoose(info.packets), _step10; !(_step10 = _iterator10()).done;) {
    var pkt = _step10.value;
    if (pkt.derived) continue;
...
function packFormat(sinfo, pkt, useBooleans) {
  ...
  for (var _iterator11 = _createForOfIteratorHelperLoose(pkt.fields), _step11; !(_step11 = _iterator11()).done;) {
    var fld = _step11.value;
...
function toTypescript(info, staticTypeScript) {
  ...
  for (var _iterator13 = _createForOfIteratorHelperLoose(info.packets), _step13; !(_step13 = _iterator13()).done;) {
    var pkt = _step13.value;
    if (pkt.derived) continue;
    var cmt = addComment(pkt);
  ...
  for (var _i5 = 0, _Object$keys5 = Object.keys(tsEnums); _i5 < _Object$keys5.length; _i5++) {
...
//# sourceMappingURL=73adcdd8a542553a7ae3e11b5d40c78a618eeada-19c6c29f05dc3ff320c6.js.map