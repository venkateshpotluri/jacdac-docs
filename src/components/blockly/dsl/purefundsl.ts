import { JD_SERIAL_MAX_PAYLOAD_SIZE } from "../../../../jacdac-ts/src/jdom/constants"
...
// pure functions that can be used as expressions

// TODO: static options (enums)

export interface PureFunctionDescription {
    name: string
    description: string
    argumentTypes: ("string" | "boolean" | "number")[]
    outputType: "string" | "boolean" | "number"
    jsImplementation: (args: (string | boolean | number)[]) => (string | boolean | number)
}

const pureFunctions: PureFunctionDescription[] = [
    {
        name: "random",
        description: "Return a pseudo random number between 0 and 1",
        argumentTypes: [],
        outputType: "number",
        jsImplementation: () => { return Math.random() }
    }
]


const pureFunDsl: BlockDomainSpecificLanguage = {
    id: "jacdacpurefun",
    createBlocks: () => [
        {
            kind: "block",
